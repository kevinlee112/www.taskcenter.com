...
class Tools_page
{
    ...
    public function get_ajax_html()
    {
        ...
        if($this->_current_page > $this->_interval)
        {
            $html .= '<li><a href="javascript:get_page(1);" data_page=1>首页</a></li>';
        }
        if($this->_current_page != $this->_prev_page)
        {
            $html .= '<li><a href="javascript:get_page('.$this->_prev_page.');" data_page=' . $this->_prev_page . '>&laquo;</a></li>';
        }
        ...
        for($i = $start; $i <= $end; $i++)
        {
            if($i == $this->_current_page)
            {
                $html .= '<li class="active"><a class="cur" href="javascript:get_page('.$i.');" data_page=' . $i . '>' . $i . '</a></li>';
            }
            else
            {
                $html .= '<li><a href="javascript:get_page('.$i.');" data_page=' . $i . '>' . $i . '</a></li>';
            }
        }
    
        if($this->_current_page != $this->_total_page)
        {
            $html .= '<li><a href="javascript:get_page('.$this->_next_page.');" data_page=' . $this->_next_page . '>&raquo;</a></li>';
        }
        if(($this->_current_page + $this->_interval) < $this->_total_page)
        {
            $html .= '<li><a href="javascript:get_page('.$this->_total_page.');" data_page=' . $this->_total_page . '>末页</a></li>';
        }
