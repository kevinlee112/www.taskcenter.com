...
class Public_Error
{
     ...
     * 砸蛋活动错误类型
     */
    const ERR_AWARD_NOT_EXIST = -9000;
    const ERR_AWARD_HAS_EXIST = -9001;
    const ERR_AWARD_CONFIG_ERROR = -9002;
    const ERR_SEARCH_TIME_ERROR = -9003;

    /**
     * 任务添加
     */
    const ERR_NOT_PERMISSION_MODIFY_TYPE = -10000;

    /**
     * APP版本
     */
    const ERR_NOT_CONFIG_HAS_EXISTED = -11000;
    ...
    private static $_errMsg = [
        ...
        //角色
        self::ERR_USER_WRONG_PASSWORD => ['密码错误！', 'wrong password'],
        ...
        //砸蛋活动操作
        self::ERR_AWARD_NOT_EXIST => ['活动信息不存在', 'award info not exist'],
        self::ERR_AWARD_HAS_EXIST => ['活动信息已存在', 'award has exists'],
        self::ERR_AWARD_CONFIG_ERROR => ['活动配置不完整', 'award config is empty'],
        self::ERR_SEARCH_TIME_ERROR => ['查询时间错误', 'search time is error'],

        // 任务模块
        self::ERR_NOT_PERMISSION_MODIFY_TYPE =>['不允许修改任务类型','The task type is not allowed to modify'],

        // 任务模块
        self::ERR_NOT_CONFIG_HAS_EXISTED =>['配置版本已存在','The config has existed'],
